...
public class Term {
   ...
   public boolean isSatisfied(Scan s) {
      ...
      switch(this.operator) {
    	  case "=":
    		  return isEqual(lhsval, rhsval);
    	  case "<":
    		  return isLessThan(lhsval, rhsval);
    	  case "<=":
    		  return isLessThanOrEqual(lhsval, rhsval);
    	  case ">":
    		  return isMoreThan(lhsval, rhsval);
    	  case ">=":
    		  return isMoreThanOrEqual(lhsval, rhsval);
    	  case "!=":
    	  case "<>":
    		  return isNotEqual(lhsval, rhsval);
    	  default:
    		  return false;
      }
   }
   
   private boolean isEqual(Constant lhsval, Constant rhsval) {
	   return rhsval.equals(lhsval);
   }
   
   private boolean isLessThan(Constant lhsval, Constant rhsval) {
	   return lhsval.compareTo(rhsval) < 0;
   }
   
   private boolean isLessThanOrEqual(Constant lhsval, Constant rhsval) {
	   return lhsval.compareTo(rhsval) <= 0;
   }
   
   private boolean isMoreThan(Constant lhsval, Constant rhsval) {
	   return lhsval.compareTo(rhsval) > 0;
   }
   
   private boolean isMoreThanOrEqual(Constant lhsval, Constant rhsval) {
	   return lhsval.compareTo(rhsval) >= 0;
   }
   
   private boolean isNotEqual(Constant lhsval, Constant rhsval) {
